...
/****** Object:  StoredProcedure [dbo].[usp_save_patron]    Script Date: 12/08/2015 11:50:33 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[usp_save_patron]
...
*/
	...
	DECLARE @v_count int
	SELECT @v_count = COUNT(ID) FROM dbo.patron WHERE username = @p_username
	
	IF @v_count > 0
	BEGIN
		RAISERROR ('USERNAME IS ALREADY IN USE', 16, 0)
		RETURN -1
	END
